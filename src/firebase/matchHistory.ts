...
import { isCpuUid } from '../bot/BotFactory';
...
export async function saveMatchResult(match: MatchRecord): Promise<void> {
  try {
    ...
    // Update winner stats (skip for CPU UIDs)
    if (!isCpuUid(match.winnerUid)) {
